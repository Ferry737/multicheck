...
export function Trainer({ getQuestions, title, showTimer, noImmediateFeedback, onDone }: TrainerProps) {
  ...
  if (i >= qs.length) {
    ...
    return (
      <main className="container-x py-10 max-w-xl text-center">
        <h1 className="text-2xl font-bold">Fertig</h1>
        <p className="mt-3 text-4xl font-bold tabular">{Math.round((corr / qs.length) * 100)}%</p>
        <p className="mt-2 text-ink-muted">{corr}/{qs.length} richtig · {Math.round(ms / 1000)}s</p>
        <button onClick={() => location.reload()} className="mt-6 rounded-xl border border-line px-6 py-3 font-medium hover:border-brand">Nochmal</button>
      </main>
    );
  }
  ...
  return (
    <main className="container-x py-6 max-w-2xl">
      ...
      <div className="mt-4 rounded-card border border-line bg-paper p-6 shadow-card">
        {q.stimulus && !hidden && (
          <div className="mb-4 flex flex-col items-center rounded-xl bg-page p-3">
            <div dangerouslySetInnerHTML={{ __html: q.stimulus }} />
            {q.type === "recall" && !revealed && (
              <button onClick={() => setHidden(true)} className="mt-3 rounded-xl border border-line px-4 py-2 text-sm font-medium hover:border-brand">
                Gemerkt — zur Frage
              </button>
            )}
          </div>
        ...
        {!revealed ? (
          <>
              ...
              <div className="mt-4 grid gap-2">
                {q.options?.map((o, idx) => (
                  <button key={o} onClick={() => setInput(o)} aria-label={`Antwort ${o}`}
                    className={`flex items-center justify-between rounded-xl border px-4 py-3 text-left text-sm ${input === o ? "border-brand bg-brand-soft" : "border-line hover:border-brand/50"}`}>
                    <span>{o}</span><span className="text-ink-faint text-xs">{idx + 1}</span>
                  </button>
                ))}
              </div>
            ) : (
            ...
            <button onClick={submit} disabled={!input} className="mt-4 w-full rounded-xl bg-brand px-5 py-3 text-white font-semibold disabled:opacity-40">
          ...
          <div>
            {!noImmediateFeedback && (
              <div className={`mt-4 rounded-xl p-4 text-sm ${correct ? "bg-good/10 text-good" : "bg-bad/10 text-bad"}`} role="status">
                {correct ? "✓ Richtig" : `✗ Richtig: ${q.answer}`}
              </div>
            )}
            {!noImmediateFeedback && <p className="mt-2 text-sm text-ink-muted">{q.explanation}</p>}
            <button onClick={() => { setResults((r) => [...r, isCorrect]); setRevealed(false); setInput(""); setI((v) => v + 1); }} className="mt-4 w-full rounded-xl bg-brand px-5 py-3 text-white font-semibold">Weiter →</button>
          </div>
        )}
      </div>
